(ns ona.api.dataset_test
  (:require [midje.sweet :refer :all]
            [ona.api.dataset :refer :all]
            [ona.utils.file :as f]
            [ona.utils.remote :refer [make-j2x-url]]
            [ona.api.http :refer [parse-http]]
            [ona.api.io :refer [make-url multipart-options]]))
...
(let [url :fake-url
  ...
  (fact "about online-data-entry-link"
        ...
        (provided
         ...
         (#'ona.api.io/http-request :get url {}) =>
         {:body :body
          :request :request
          :status :status}
         (#'ona.api.io/add-to-options account suppress-40x url) => {}
         (ona.api.io/parse-response :body :status nil nil) => {:enketo_url :enketo_url}))
